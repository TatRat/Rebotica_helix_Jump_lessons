...
public class Ball : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision other) //вызывается при соприкосновении объекта с другим коллайдером
    {
        ...
        else if (other.gameObject.CompareTag("End")) //При достижении конца уровня
        {
            GetComponent<GameManager>().NewLevel(); //Изменение уровня игрока
            SceneManager.LoadScene(0); //Загрузка сцены
        }
        else if (other.gameObject.CompareTag("Dangerous_Sector")) //При соприкосновении с опасными секторами
        {
